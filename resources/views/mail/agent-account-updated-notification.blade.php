...
<html lang="pl">
    <head>
        <meta charset="utf-8">
        <title>Zmieniono dane konta</title>
    </head>
                    ...
                    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:640px;background:#ffffff;border:1px solid #e5e7eb;border-radius:8px;">
                        <tr>
                            <td style="padding:24px 24px 12px;">
                                ...
                                <h1 style="margin:0;color:#111827;font-size:22px;">Zmieniono dane Twojego konta</h1>
                            </td>
                        </tr>
                        <tr>
                            <td style="padding:12px 24px;">
                                <p style="margin:0 0 16px;font-size:15px;line-height:1.6;">
                                    Administrator zaktualizowal dane konta <strong>{{ $agent->name }}</strong>.
                                </p>
                                ...
                                @if($changes !== [])
                                    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="font-size:14px;line-height:1.6;border-collapse:collapse;">
                                        @foreach($changes as $field => $change)
                                            <tr>
                                                <td style="padding:6px 0;color:#6b7280;">{{ $field }}</td>
                                                <td style="padding:6px 0;text-align:right;font-weight:700;">{{ $change['from'] ?? '-' }} -> {{ $change['to'] ?? '-' }}</td>
                                            </tr>
                                        @endforeach
                                    </table>
                                @endif
                                ...
                                @if($passwordChanged)
                                    <p style="margin:16px 0 0;border-left:4px solid #4f46e5;background:#eef2ff;padding:12px 14px;border-radius:4px;font-size:14px;line-height:1.6;">
                                        Haslo do konta zostalo zmienione. Przy kolejnym logowaniu uzyj nowego hasla przekazanego przez administratora.
                                    </p>
                                @endif
                            </td>
                        </tr>

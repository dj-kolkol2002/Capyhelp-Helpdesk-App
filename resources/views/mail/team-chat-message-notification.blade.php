<!DOCTYPE html>
<html lang="pl">
    <head>
        <meta charset="utf-8">
        <title>Nowa wiadomosc czatu</title>
    </head>
    <body style="margin:0;background:#f4f6f8;font-family:Arial,sans-serif;color:#1f2937;">
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f4f6f8;padding:24px;">
            <tr>
                <td align="center">
                    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:640px;background:#ffffff;border:1px solid #e5e7eb;border-radius:8px;">
                        <tr>
                            <td style="padding:24px 24px 12px;">
                                <p style="margin:0 0 8px;color:#2563eb;font-size:14px;font-weight:700;">CAPYHELP</p>
                                <h1 style="margin:0;color:#111827;font-size:22px;">Nowa wiadomosc od {{ $teamChatMessage->user->name }}</h1>
                            </td>
                        </tr>
                        <tr>
                            <td style="padding:12px 24px;">
                                <div style="border-left:4px solid #0ea5e9;background:#f0f9ff;padding:14px 16px;border-radius:4px;font-size:15px;line-height:1.6;white-space:pre-line;">{{ $teamChatMessage->body }}</div>
                                @if($teamChatMessage->attachments->isNotEmpty())
                                    <p style="margin:16px 0 0;color:#6b7280;font-size:13px;">
                                        Zalaczniki: {{ $teamChatMessage->attachments->pluck('original_name')->join(', ') }}
                                    </p>
                                @endif
                            </td>
                        </tr>
                        <tr>
                            <td style="padding:12px 24px 24px;">
                                <a href="{{ route('dashboard') }}" style="display:inline-block;background:#2563eb;color:#ffffff;text-decoration:none;border-radius:6px;padding:10px 16px;font-size:14px;font-weight:700;">
                                    Otworz panel
                                </a>
                            </td>
                        </tr>
                    </table>
                </td>
            </tr>
        </table>
    </body>
</html>
